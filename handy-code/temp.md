{% for state in states.binary_sensor if 'vmtbsh02_ha_dockermon' in state.entity_id %}{% set string = state.entity_id.split('.')[1] %}{% set server = string.split('_')[0] %}{% set container = string.replace(server + "_ha_dockermon_","") %}
